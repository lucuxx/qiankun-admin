      ...
      >
        ...
        <div class="card-body">
          ...
          <el-table
            ...
            :row-class-name="
              ({ rowIndex }) =>
                rowIndex % 2 === 0 ? 'warning-row' : 'success-row'
            "
          >
            <el-table-column
              align="center"
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    v-if="i.prop === 'delete'"
                    type="danger"
                    @click="deleteClass(scope)"
                    >删除
                  </el-button>
                  <el-button
                    v-if="i.prop === 'edit'"
                    type="primary"
                    @click="openEditClass(scope)"
                    >修改
                  </el-button>
                </template>
              </el-table-column>
...
export default class addClass extends Vue {
  ...
  get body() {
    ...
    return this.$lo.map(
      this.classData,
      (item: Types.ArticleClassData, index: number) => {
  ...
  async mounted() {
    this.classData = await this.$api
      .getArticleClass()
      .then((req: Types.InterfaceData) => this.$util.checkResp(req))
    setTimeout(() => {
      this.loading = false
    }, 1000)
  }
