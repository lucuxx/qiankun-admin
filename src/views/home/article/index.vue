        ...
        <div slot="header" class="clearfix">
          <ul>
            <li
              ...
              :class="currentIndex === index ? 'color' : ''"
              :key="index"
              @click="changeClassId(item.id, index)"
            >
              {{ item.name }}
            </li>
            <el-button style="float: right" type="primary" @click="goAddArticle"
              >新增文章
            </el-button>
          </ul>
        </div>
                ...
                <div class="bottom">
                  <span class="class">{{
                    _.get(
                      _.filter(
                        classData,
                        (classItem) => classItem.id === item.tags
                      ),
                      '0.name'
                    )
                  }}</span>
                  <span
                    ...
                    class="time"
                    style="color: #37c6c0"
                    >{{ $util.checkTime(item.updateTime) }}</span
                  >
                  <span v-else class="time" style="color: #37c6c0">{{
                    item.createdTime
                  }}</span>
                  <span
                    class="more"
                  ...
                  <span
                    class="more"
                    style="color: #37c6c0"
                    @click.stop="editArticle(item.id)"
                    >编辑</span
                  >
                </div>
              </div>
...
export default class articleNav extends Vue {
  ...
  private async mounted() {
    // 拿到文章分类
    this.classData = await this.getArticleClass().then(
      (req: Types.InterfaceData) => this.$util.checkResp(req)
    )
    this.currentClassId = this.$lo.get(
      this.classData,
      `${this.currentIndex}.id`
    )
    const allArticle = await this.getArticle().then(
      (req: Types.InterfaceData) => this.$util.checkResp(req)
    )
  ...
  delArticle() {
    ...
    this.$api
      .deleteArticle({
        articleId: id,
        classId: this.classData[this.currentIndex].id,
      })
      .then((req: any) => this.$util.checkResp(req))
      .then((req: any) => {
        setTimeout(() => {
  ...
  async changeClassId(params: number, index: number) {
    ...
    this.articleInfo = await this.getArticle().then(
      (req: Types.InterfaceData) => this.$util.checkResp(req)
    )
    setTimeout(() => {
      this.loading = false
