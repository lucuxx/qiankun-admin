<template>
  <section class="addArticle" style="margin-top: 20px">
    <el-card
      class="card"
      :body-style="{ padding: '15px', height: '100%', boxSizing: 'border-box' }"
    >
      ...
      <div class="card-body">
        <el-form :model="form" label-width="80px" :rules="rules" ref="ruleForm">
          <el-form-item label="封面图片" style="height: 130px;">
            ...
            >
              <el-icon
                style="text-align: center;line-height: 120px;width: 100%;font-size: 30px;"
                class="el-icon-plus"
              ></el-icon>
            </div>
            <img
          ...
          <el-form-item>
            <el-button v-if="isAdd" type="primary" @click="onSubmit('ruleForm')"
              >新建</el-button
            >
            <el-button v-else type="primary" @click="editArticle"
              >保存</el-button
            >
...
export default class addArticle extends Vue {
  ...
  async mounted() {
    ...
    // 如果是新增
    if (this.isAdd) {
      const [classData, articleTags] = await Promise.all([
        this.getArticleClass(),
        this.getArticleTags(),
      ]).then((req) => {
        return this.$lo.map(req, (item: Types.InterfaceData) =>
          this.$util.checkResp(item)
        )
      })
      this.classData = classData
      this.articleTags = articleTags
    } else {
      const [classData, articleDetails, articleTags] = await Promise.all([
        this.getArticleClass(),
  ...
  uploadRequest(file: any) {
    const result =
      'https://github.com/luosijie/Front-end-Blog/blob/master/img/logo_vmmarkdown_name.png?raw=true'
