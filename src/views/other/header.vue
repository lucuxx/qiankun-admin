<template>
  ...
  <!-- <ul class="ul">
      ...
      >
        <router-link
          class="routerJump"
          :class="currentIndex === index ? 'selection' : ''"
          :to="item.path"
          >{{ item.name }}
        </router-link>
      </li>
      ...
      <li v-if="isLogin" class="li" style="float: right">
        <el-dropdown style="width: 100%;height: 100%;" placement="top-end">
          <el-button
            style="width: 100%;height: 100%;font-size: 18px;color: white"
            type="text"
            >{{ userInfo.name }}
          </el-button>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item
              @click.native="$router.push({ path: '/personal' })"
...
export default class HeaderNav extends Vue {
  ...
  async mounted() {
    if (JsCookie.get('email')) {
      ...
      const InterfaceData: Types.InterfaceData = await this.$api.getUserInfo({
        cookie,
      })
  ...
  changeIndex(params: Types.RouterData): void {
    this.currentIndex = this.$lo.findIndex(
      this.routerData,
      (item: Types.RouterData): boolean => item.path === params.path
    )
  }
